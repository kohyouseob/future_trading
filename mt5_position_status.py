...
def send_status_telegram_current_account() -> bool:
    """현재 MT5에 연결된 계좌 기준으로 포지션 현황을 수집해 텔레그램 전송. (포지션 모니터에서 매시 :00/:15/:30/:45 호출용)"""
...
def main():
    ...
    # 2. 계좌별 순회: 메시지 수집 후 한 번에 전송 (2초 간격 여러 건 전송 방지)
    messages = []
    for acc_config in ACCOUNTS:
        ...
        if tr.login_mt5(acc_config['login'], acc_config['password'], acc_config['server']):
            status = get_position_status()
            if status and status['account']['balance'] > 0:
                messages.append(format_status_message(status))
                acc = status['account']
                log_cronjob(f"MT5-Pos-{acc['login']}", f"✅ P/L ${acc['profit']:+,.0f}")
                print(f"✅ {acc_config['login']} 데이터 수집 완료")
            else:
                print(f"❌ {acc_config['login']} 데이터 수집 실패 또는 잔고 없음")
